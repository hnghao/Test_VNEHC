...
{
  showINA();
  uint8_t tempValue = analogRead(A0)>200?1:0;
  digitalWrite(PIN_FET_N, tempValue);
  if(tempValue == 0)
  {
    delay(5000); // WORK!
  }
...
void showINA()
{
  static unsigned long tempLastMillis;

  if(millis() - tempLastMillis >= 100)
  {
    ...
    for (int ch = 0; ch < 1; ch++)
    {
      ...
      // Serial.println(F("123") + String(13));
